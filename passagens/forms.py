...
class PassagemForms(forms.ModelForm):
    ...
    #origem = forms.CharField(label='Origem', max_length=100, initial='Salvador')
    #destino = forms.CharField(label='Destino', max_length=100, initial='Aracaju')
    #data_ida = forms.DateField(label='Ida', widget=DatePicker())
    #data_volta = forms.DateField(label='volta', widget=DatePicker())
    
    #classe_viagem = forms.ChoiceField(label='Classe do vôo', choices=tipos_de_classe)
    #informações = forms.CharField(label='informações extras', max_length=200, widget=forms.Textarea(), required=False)
    #email = EmailField(label='Email', max_length=150, initial='teste@gmail.com')
